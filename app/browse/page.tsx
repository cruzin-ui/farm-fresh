...
import { Search, Filter, Calendar, Tag, Sprout, ChevronRight, AlertCircle, MapPin } from 'lucide-react';
...
export default function BuyerFeed() {
  ...
  const filteredListings = listings.filter((item) => {
    ...
    const matchesZip = 
      zipSearch === '' || 
      (item.zip_code && item.zip_code.includes(zipSearch)) ||
      (item.location_name && item.location_name.toLowerCase().includes(zipSearch.toLowerCase()));
    ...
    return matchesSearch && matchesZip && matchesCategory && matchesPrice && matchesReady;
  });
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-8 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="md:col-span-2 relative">
            ...
            <input
              type="text"
              placeholder="Search heirloom tomatoes, honey..."
              value={searchTerm}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none text-sm"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full py-2 px-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none bg-white text-gray-700 text-sm"
            >
              <option value="all">All Dates</option>
              <option value="today">Ready Today</option>
              <option value="upcoming">Pre-Orders</option>
            </select>
          </div>
        </div>
      ...
      {!loading && filteredListings.length === 0 && (
        <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Try adjusting your location, price range, or search keywords.
          </p>
        </div>
      )}
                ...
                <div className="p-4">
                  ...
                  {item.location_name && (
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                      <MapPin className="w-3 h-3 text-gray-400" /> {item.location_name} {item.zip_code ? `(${item.zip_code})` : ''}
                    </p>
                  )}
