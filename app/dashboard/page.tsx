...
export default function SellerDashboardPage() {
  ...
  const handleListingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Automatically attach farm_name and avatar_url from profile state
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-64 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm shrink-0 self-start">
          <div className="flex items-center gap-3 pb-6 mb-6 border-b border-gray-100">
            {avatarUrl ? (
              <img src={avatarUrl} alt={farmName} className="w-11 h-11 rounded-xl object-cover border border-emerald-200" />
            ) : (
        ...
        <main className="flex-1 bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm">
              ...
              {activeTab === 'new' && (
                <form onSubmit={handleListingSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Ready Date *</label>
                      <input
                        type="date"
                        required
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">Crop Image (Optional)</label>
                    <input
                      type="file"
                      accept="image/*"
          ...
          {/* TAB 2: FARM PROFILE & PHOTO */}
          {activeTab === 'profile' && (
            <form onSubmit={handleProfileSubmit} className="space-y-6">
              <div className="pb-4 border-b border-gray-100">
                <h1 className="text-2xl font-bold text-gray-900">Your Farm Profile & Branding</h1>
                <p className="text-xs text-gray-500 mt-0.5">Set your farm photo and name to appear automatically on all postings.</p>
              </div>

              {/* FARM / FARMER PHOTO UPLOAD */}
              <div className="flex items-center gap-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
                {avatarUrl ? (
                  <img src={avatarUrl} alt="Farm avatar" className="w-20 h-20 rounded-2xl object-cover border-2 border-emerald-500 shadow-sm shrink-0" />
                ) : (
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Upload Farm / Farmer Photo</label>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  />
                  <p className="text-[10px] text-gray-400 mt-1">This photo will display beside every harvest listing you publish.</p>
                </div>
              </div>
